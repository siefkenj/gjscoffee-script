...
(function() {
  var Access, Arr, Assign, AssignConst, Base, Block, Call, Class, Closure, Code, Comment, Existence, Extends, For, GClass, IDENTIFIER, IDENTIFIER_STR, IS_STRING, If, Import, In, Index, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, Literal, METHOD_DEF, NEGATE, NO, Obj, Op, Param, Parens, RESERVED, Range, Return, SIMPLENUM, STRICT_PROSCRIBED, Scope, Slice, Splat, Switch, TAB, THIS, Throw, Try, UTILITIES, Value, While, YES, compact, del, ends, extend, flatten, last, merge, multident, starts, unfoldSoak, utility, _ref, _ref1,
  ...
  exports.Call = Call = (function(_super) {
    ...
    function Call(variable, args, soak) {
      ...
      this.isLocalBind = variable === 'localbind';
      this.variable = this.isSuper || this.isLocalBind ? null : variable;
    }
    ...
    Call.prototype.compileNode = function(o) {
      ...
      } else if (this.isLocalBind) {
        return "Lang.Bind(this, " + args + ")";
      } else {
  ...
  exports.GClass = GClass = (function(_super) {

    __extends(GClass, _super);

    function GClass(variable, parent, body) {
    ...
    GClass.prototype.children = ['variable', 'parent', 'body'];

    GClass.prototype.determineName = function() {
      var decl, tail;
      if (!this.variable) {
        return null;
      }
    ...
    GClass.prototype.setContext = function(name) {
    ...
    GClass.prototype.addBoundFunctions = function(o) {
      var bvar, initBind, lhs, _i, _len, _ref2;
      if (this.boundFuncs.length) {
        this.ctorBind = new Code;
        this.ctorBind.noReturn = true;
        _ref2 = this.boundFuncs;
        for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
          bvar = _ref2[_i];
          lhs = (new Value(new Literal("this"), [new Access(bvar)])).compile(o);
          this.ctorBind.body.unshift(new Literal("" + lhs + " = " + lhs + ".bind(this)"));
        }
        if (!this.ctor) {
          this.ctor = this.makeConstructor();
          this.body.expressions.unshift(new Assign(new Literal('_init'), this.ctor, 'object'));
        }
        initBind = new Assign(new Literal('_initBind'), this.ctorBind, 'object');
        this.body.expressions.unshift(initBind);
        return this.ctor.body.unshift(new Literal('this._initBind.call(this)'));
      }
    };

    GClass.prototype.addProperties = function(node, name, o) {
      ...
      exprs = (function() {
        ...
        while (assign = props.shift()) {
          if (assign instanceof Assign) {
            ...
            func = assign.value;
            if ((_ref2 = base.value) === 'constructor' || _ref2 === '_init') {
              if (this.ctor) {
              ...
              if (func instanceof Code) {
                ...
                this.externalCtor = func;
                this.ctor = this.makeConstructor();
                assign.value = this.ctor;
              }
            } else {
    ...
    GClass.prototype.makeConstructor = function() {
      var ctor;
      ctor = new Code;
      ctor.noReturn = true;
      if (this.externalCtor) {
        ctor.body.push(new Literal("" + (this.externalCtor.compileNode(null)) + ".apply(this, arguments)"));
      }
      return ctor;
    };

    GClass.prototype.walkBody = function(name, o) {
      var _this = this;
    ...
    GClass.prototype.createMethodListObject = function() {
      var a;
      return (function() {
        var _i, _len, _ref2, _results;
    ...
    GClass.prototype.compileNode = function(o) {
      var decl, klass, lname, methodList, methodsObj, name;
      ...
      if (this.parent) {
        methodList.push(new Assign(new Literal('Extends'), this.parent, 'object'));
      }
      methodList = methodList.concat(this.createMethodListObject());
    ...
    Class.prototype.addBoundFunctions = function(o) {
      ...
      if (this.boundFuncs.length) {
        ...
        for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
          bvar = _ref2[_i];
          lhs = (new Value(new Literal("this"), [new Access(bvar)])).compile(o);
          _results.push(this.ctor.body.unshift(new Literal("" + lhs + " = " + lhs + ".bind(this)")));
        }
        return _results;
      }
    };
  ...
  exports.AssignConst = AssignConst = (function(_super) {

    __extends(AssignConst, _super);

    function AssignConst(variable, value, options) {
    ...
    AssignConst.prototype.children = ['variable', 'value'];

    AssignConst.prototype.isStatement = function(o) {
      return false;
    };

    AssignConst.prototype.assigns = function(name) {
      return this[this.context === 'object' ? 'value' : 'variable'].assigns(name);
    };

    AssignConst.prototype.unfoldSoak = function(o) {
      return unfoldSoak(o, this, 'variable');
    };

    AssignConst.prototype.makeReturn = function(res) {
      throw new SyntaxError("\"const " + (this.variable.compile(res)) + " = ...\" is a pureStatement and cannot be used in a return statement.");
    };

    AssignConst.prototype.compileNode = function(o) {
      var match, name, val, varBase, _ref2, _ref3, _ref4;
      name = this.variable.compile(o, LEVEL_LIST);
      if (!this.context) {
        if (!(varBase = this.variable.unwrapAll()).isAssignable()) {
          throw SyntaxError("\"" + (this.variable.compile(o)) + "\" cannot be assigned in a const expression.");
        }
      ...
      if (this.value instanceof Code && (match = METHOD_DEF.exec(name))) {
        ...
        this.value.name = (_ref2 = (_ref3 = (_ref4 = match[2]) != null ? _ref4 : match[3]) != null ? _ref3 : match[4]) != null ? _ref2 : match[5];
      }
      val = this.tab + ("const " + name + " = " + (this.value.compile(o, LEVEL_LIST)));
      return val;
    };
  ...
  exports.Import = Import = (function(_super) {
    ...
    function Import(importPath, asIdentifier) {
      var lastAccessor, properties;
      this.importPath = importPath;
      this.asIdentifier = asIdentifier;
      if (this.asIdentifier) {
        this.asIdentifier = this.asIdentifier.compile();
      } else {
        if (this.importPath.properties.length > 0) {
          lastAccessor = this.importPath.properties[this.importPath.properties.length - 1];
          if (!lastAccessor.name) {
            throw new SyntaxError("\"" + (this.importPath.compile()) + "\" is not a valid import path.");
          }
          this.asIdentifier = lastAccessor.name.value;
        } else {
          this.asIdentifier = this.importPath.base.value;
        }
      }
      properties = this.importPath.properties;
      properties.unshift(new Access(new Literal(this.importPath.base.value)));
      this.importPath = new Value(new Literal('imports'), properties);
      return new AssignConst(new Literal(this.asIdentifier), this.importPath);
    }

    Import.prototype.children = ['importPath', 'asIdentifier'];
