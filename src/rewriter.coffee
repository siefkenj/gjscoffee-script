...
class exports.Rewriter
  ...
  addImplicitParentheses: ->
    ...
    condition = (token, i) ->
      ...
      seenSingle  = yes if tag in ['IF', 'ELSE', 'CATCH', '->', '=>', 'CLASS', 'GCLASS']
      ...
      not token.generated and @tag(i - 1) isnt ',' and (tag in IMPLICIT_END or
        ...
        (tag isnt 'INDENT' or
          (@tag(i - 2) not in ['CLASS', 'GCLASS', 'EXTENDS'] and @tag(i - 1) not in IMPLICIT_BLOCK and
    ...
    @scanTokens (token, i, tokens) ->
      tag     = token[0]
      noCall  = yes if tag in ['CLASS', 'GCLASS', 'IF', 'FOR', 'WHILE']
...
IMPLICIT_FUNC    = ['IDENTIFIER', 'SUPER', ')', 'CALL_END', ']', 'INDEX_END', '@', 'THIS', 'LOCALBIND']

# If preceded by an `IMPLICIT_FUNC`, indicates a function invocation.
IMPLICIT_CALL    = [
  'IDENTIFIER', 'NUMBER', 'STRING', 'JS', 'REGEX', 'NEW', 'PARAM_START', 'CLASS', 'GCLASS'
  'IF', 'TRY', 'SWITCH', 'THIS', 'BOOL', 'UNARY', 'SUPER', 'LOCALBIND'
  '@', '->', '=>', '[', '(', '{', '--', '++'
]
